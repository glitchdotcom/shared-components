...
const items = ['spotify', 'spotify-web-playback', 'spotify-audio-features', 'spotify-playground', 'spotify-oauth', 'spotify-token-swap'].map(
  (id) => ({ id }),
);
...
export const StoryGrid = () => (
  <>
    <p>
      The Grid component renders an array of items as a responsive grid. Instead of having a fixed number of columns, the grid will render as many
      columns per row as it can while keeping their width above "minWidth".
    </p>
    <CodeExample>
      {code`
    ...
    <PropsDefinition>
      ...
      <Prop name="gap">The width of the gap between items. Defaults to --space-1.</Prop>
      <Prop name="minWidth">The minimum width of a column. Defaults to 18rem.</Prop>
      <Prop name="itemProps">
...
export const StoryGrid_sizes = () => (
  <>
    <p>Grids adjust to fit their container, not the viewport. Here's the same grid with a minWidth of 230px in different containers:</p>
    <h3>1200px</h3>
    <GridWithItems />
    <h3>1000px</h3>
    ...
    <p>
      Note that if the container is <em>smaller</em> than the mininmum width, elements will overflow:
    </p>
    <h3>200px</h3>
