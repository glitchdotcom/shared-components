...
export const StorySearchResults = () => {
  ...
  const filteredOptions = React.useMemo(
    () => resultOptions.filter((opt) => opt.domain.includes(query.trim())).concat({ type: 'showAllResults' }), 
    [query]);
  ...
  return (
    <>
      <p>The SearchResults component combines a search TextInput with a ResultsList to make a search box with result previews.</p>
      <CodeExample>
        {code`
      ...
      <form onSubmit={searchForQuery}>
        <SearchResults label="projects search" value={query} onChange={(q) => setQuery(q)} options={filteredOptions}>
          {({ item, buttonProps }) => item.type === 'project' ? (
            ...
            <ResultItem as="a" href={`https://glitch.com/~${item.domain}`} {...buttonProps}>
              <ResultInfo>
                <ResultName>{Show all }</ResultName>
                {item.description && <ResultDescription>{item.description}</ResultDescription>}
              </ResultInfo>
            </ResultItem>
          )}
