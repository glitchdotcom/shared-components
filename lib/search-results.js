...
import { useClickOutside } from './popover'
...
const reducers = {
  ...
  clickedOutside: (state) => ({ ...state, shouldShowResults: false }),
}
...
export const SearchResults = ({ value, options, onChange, onKeyDown, children, ...props }) => {
  ...
  const rootRef = useClickOutside(shouldShowResults, () => {
    dispatch('clickedOutside');
  });

    
  return (
    <Container ref={rootRef} data-module="SearchResults">
      <TextInput
        type="search" 
        variant="opaque"
        value={value} 
