...
export const StorySearchResults = () => {
  ...
  return (
    <>
      <p>
        The SearchResults component combines a search TextInput with a ResultsList to make a search box with result previews. 
        Tab toggles from the search box to the results, and arrow keys navigate the results.
      </p>
      <CodeExample>
        {code`
      ...
      <PropsDefinition>
        ...
        <Prop name="options" required>
          ...
          You will probably want to generate or filter this list based on the query, but filtering is not handled by this component.
        </Prop>
        <Prop name="children">
...
export const StorySearchResults_with_multiple_result_types = () => {
  ...
  return (
    <>
      ...
      <form onSubmit={searchForQuery}>
        <SearchResults label="projects search" value={query} onChange={(q) => setQuery(q)} options={filteredOptions}>
          {({ item, buttonProps }) => item.type === 'project' ? (
            <ResultItem as="a" href={`https://glitch.com/~${item.domain}`} {...buttonProps}>
              <ResultInfo>
                <ResultName>{item.domain}</ResultName>
                {item.description && <ResultDescription>{item.description}</ResultDescription>}
