...
const MultiPopoverContext = React.createContext();
...
export const MultiPopover = ({ views, initialView, children }) => {
  ...
  return activeViewFunc({ activeView, setActiveView });
};

export const StoryMultiPopoverContent = () => (
  <PopoverMenu label={<Button as="span">Add Project to Collection</Button>}>
    {({ open, onClose }) => (
      <MultiPopover
        views={{
          ({ setActiveView }) => 
...
export const MultiPopoverTitle = ({ children }) => {
  const { setActiveView, defaultView } = React.useContext(MultiPopoverContext);
  return (
