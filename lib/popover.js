...
const useMenuOptions = ({ open, options, onClose }) => {
  ...
  const optionsWithProps = options.map((opt, index) => ({
    ...opt,
  ...
  return optionsWithHandlers
}

export const PopoverActionsList = ({ options, open, onClose, ...props }) => {
  const optionsWithHandlers = useMenuOptions({ open, options, onClose })
  ...
  return (
    <PopoverContent data-module="PopoverActionsList" {...props}>
      <PopoverActionsUL as="ul">
        {optionsWithHandlers.map((option, index) => (
