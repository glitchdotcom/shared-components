...
export const PopoverMenu = ({ label, buttonProps = {}, initialView, children, ...props }) => {
  const [activeView, setActiveView] = React.useState(null);
  
  const views = typeof children === 'function' ? 
  ...
  return (
    <PopoverContainer data-module="PopoverMenu" ref={rootRef} onToggle={onToggle} open={open} role="menu" {...props}>
      ...
      {activeViewFunc({ open, activeView, setActiveView, onClose: () => setOpen(false) })}
    </PopoverContainer>
  );
};
...
export const StoryPopoverMenu_Multiple_Views = () => (
  <PopoverMenu initialView="main" label={<Button as="span">Open Multi-Page Popover</Button>}>
    {{
      foo: ({ setActiveView, onClose }) => (
        <PopoverContent align="left" size="wide">
          <Title onBack={() => setActiveView(null)} onClose={onClose}>
            Foo
          </Title>
          <Info>Foo content goes here</Info>
        </PopoverContent>
      ),
      bar: ({ setActiveView, onClose }) => (
        <PopoverContent align="left" size="wide">
          <Title onBack={() => setActiveView(null)} onClose={onClose}>
            Bar
          </Title>
      ...
      <PopoverActionsUL as="ul">
        {optionsWithMenuProps.map((option) => (
          <PopoverLI key={option.id}>
            <Button size="tiny" variant="secondary" onClick={(e) => closeAndChange(option, e)} {...option.menuProps}>
              {option.label}
            </Button>
          </PopoverLI>
        ))}
