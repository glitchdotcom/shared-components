...
export const StoryButton_as_link = () => (
  <>
    <p>
      Button styles can be applied to other elements, such as <code>{`<a>`}</code> or <code>{`<summary>`}</code>. 
      We can override the button's element with the "as" prop available on all styled components.
    </p>
    <CodeExample>{`<Button as="a" href="https://glitch.com/edit/#!/shared-components">View Source</Button>`}</CodeExample>
    <Container>
      <Button as="a" href="https://glitch.com/edit/#!/shared-components">View Source</Button>
    </Container>
  </>
)
...
export const StoryButton_as_decorative_element = () => (
  <>
    <p>
      ...
      In this case, we can use the "as" prop to use a non-interactive element for the button.
      This decorative button will have its hover styles applied when you hover over the <em>parent</em> element.
    </p>
