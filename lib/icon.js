...
// ['bentoBox', 'bomb', 'clapper', 'herb', 'dogFace', 'framedPicture']
...
export const Icon = ({ icon, alt, ...props }) => (
  <IconBase data-module="icon" src={icons[icon]} alt={alt} {...props} />
)
Icon.propTypes = {
...
export const story_Icons_in_Buttons = () => (
  <>
    <Container>
    ...
    <Container>
      <Button size="small" onClick={() => console.log(null)}>No Icon</Button>
      {Object.keys(icons).map((icon) => (
        <Button key={icon} size="small" onClick={() => console.log(icon)}>
          {icon} <Icon icon={icon} />
        </Button>
      ))}
    </Container>
