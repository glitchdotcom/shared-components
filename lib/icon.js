...
// monochromatic SVGs are handled separately from emoji so that their color can be controlled.
// should these be handled by a separate component?
const svgs = {
  arrowRight: (props) => (
    <IconBase as="svg" viewBox="0 0 64 64" {...props}>
      <polygon points="64,32 48,48 48,36 0,36 0,28 48,28 48,16" fill="currentColor"/>
    </IconBase>
  ),
  sunglasses: (props) => (
    <IconBase as="svg" viewBox="0 0 29 15" {...props}>
  ...
  octocat: (props) => (
    <IconBase as="svg" viewBox="-291 293 16 15">
...
export const Icon = ({ icon, alt, ...props }) => {
  if (svgs[icon]) {
    return React.createElement(svgs[icon], alt ? { 'aria-label': alt } : { 'aria-hidden': true });
  }
  
  return <IconBase data-module="Icon" src={icons[icon]} alt={alt} {...props} />; 
}
Icon.propTypes = {
