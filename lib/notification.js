...
const UploadNotification = ({ value, onRemove }) => {
  const { value, cancel } = useProgress()
  
  return (
    <Notification aria-live="polite" onRemove={onRemove}>
      <Progress value={value} max={100} style={{ display: 'inline-block', width: '4rem'}}>{value}%</Progress>
      &nbsp;
      {value < 100 ? 'Uploading…' : 'Uploaded!'}
    </>
  )
}
const Content = () => {
  ...
  const createLoadingNotification = () => {
    ...
    const notification = createNotification(
      ...
      { lifetime: 'persistent', onRemove: () => { progress.cancel(); notification.removeNotification() } }
    )
    
    progress.onChange((value) => notification.updateNotification(
      <UploadNotification value={value} onRemove={() => { notification.removeNotification() }} />
    ))
  }
