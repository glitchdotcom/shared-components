...
export const NotificationsProvider = ({ children }) => {
  ...
  const funcs = React.useMemo(() => {
    const createNotification = React.useCallback((content, opts = {}) => {
      const notification = {
      ...
      if (notification.lifetime === "persistent") {
    ...
    const createErrorNotification = (content = 'Something went wrong. Try refreshing?', opts = {}) => 
      createNotification(content, { variant: "error", ...opts });
  ...
  return (
    <>
      <Provider value={funcs}>{children}</Provider>
