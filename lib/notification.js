...
export const Notification = ({ message, live, children, variant, timeout, onClose, ...props }) => {
  ...
  React.useEffect(() => {
    if (!announce) announce({ message: `${variant}: ${message}`, live })
  }, [variant, message, live])
...
const NotificationToggle = ({ children }) => {
  const [showNotification, setShow] = React.useState(true);
  return showNotification ? children(() => setShow(false)) : <Button onClick={() => setShow(true)}>Show Notification</Button>;
};
...
export const StoryNotification = () => (
  <>
    ...
    <PropsDefinition>
      <Prop name="message" required>The text of the notification.</Prop>
      <Prop name="children">
        An optional rich-content version of the notification that can include formatting and interactive elements.
      </Prop>
      <Prop name="live">The aria-live behavior of the notification: "polite" or "assertive" (default "polite").</Prop>
      <Prop name="variant">The style of notification: "notice", "success", "error" (default "notice").</Prop>
      <Prop name="timeout">
      ...
      <Prop name="onClose">
        A callback function, called when the close button is clicked or the notification times out. If no value is provided, the notification will not
        render a close button.
      </Prop>
    </PropsDefinition>
    <h3>variant="notice"</h3>
    <NotificationToggle>
      {(onClose) => (
        <Notification variant="notice" onClose={onClose} message="Reconnecting…" />
      )}
    </NotificationToggle>
    ...
    <NotificationToggle>
      {(onClose) => (
        <Notification variant="success" onClose={onClose} message="Added power-passenger to collection linen-collection">
          <p style={{ marginTop: 0, marginBottom: 'var(--space-1)' }}>Added power-passenger to collection linen-collection</p>
    ...
    <NotificationToggle>
      {(onClose) => (
        <Notification variant="error" onClose={onClose} message="Something went wrong. Try refreshing?" />
      )}
    </NotificationToggle>
  </>
);
  ...
  const contextValue = React.useMemo(() => {
    ...
    const createErrorNotification = (message = 'Something went wrong. Try refreshing?') => {
      createNotification((props) => (
        <Notification variant="error" timeout={2500} live="polite" message={message} {...props} />
      ));
    };
...
export const StoryNotificationsProvider_and_useNotifications = () => (
  <>
    ...
    <CodeExample>{code`
      ...
      createNotification((props) => (
        <Notification variant="success" message="Added power-passenger to collection linen-collection" {...props}>
