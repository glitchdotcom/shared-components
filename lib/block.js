...
export const Title = ({ children, onBack, onClose, ...props }) => (
  <TitleWrap data-module="Title" {...props}>
    {onBack && <IconButton onClick={onBack} icon="chevronLeft" label="Back" />}
    <TitleContent>{children}</TitleContent>
    {onClose && <IconButton onClick={onClose} icon="x" label="Close" />}
  </TitleWrap>
);
