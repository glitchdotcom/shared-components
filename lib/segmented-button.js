...
const handleKeyDown = (options, option, onChange) => (e) => {
  let offset = 0;
  ...
  console.log(index, offset, nextIndex)
  const nextID = options[nextIndex].id
  onChange(nextID, e)
}

export const SegmentedButton = ({ value, options, onChange, size }) => (
  <ButtonGroup data-module="SegmentedButton" role="radiogroup">
    {options.map(option => (
      <Button
        key={option.id} 
        ...
        onKeyDown={handleKeyDown(options, option, onChange)}
      >
        {option.label}
      </Button>
    ))}
