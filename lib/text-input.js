...
const usePasswordState = () => {
  ...
  const validated = validate(password, repeatPassword);
  ...
  const onBlurPassword = React.useCallback(() => {
    setError(validated);
  }, [validated]);
  
  return [
    { value: password, onChange: (val) => setPassword(val), onBlur: onBlurPassword, error },
    { value: repeatPassword, onChange: (val) => setRepeatPassword(val), onBlur }
  ]
}
