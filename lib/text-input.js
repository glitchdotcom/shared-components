...
const inputStyles = css`
  ...
  &:focus {
    box-shadow: 0px 0px 3px 0px var(--colors-focused);
  }
  ${({ variant }) =>
    ({
      opaque: css`
  ...
  <>
    ...
    <PropsDefinition>
      <Prop name="label" required>
        The label for this field as it appears to screen readers and (by default) as placeholder text.
      </Prop>
      ...
      <Prop name="type">"text", "email", "password", or "search" (default "text").</Prop>
      <Prop name="variant">The input's visual style: "underline" or "opaque" (default "underline").</Prop>
      <Prop name="placeholder">Custom text for the placeholder (defaults to the "label" value).</Prop>
      <Prop name="error">Content to be rendered as an error message.</Prop>
      <Prop name="prefix">Content to be rendered to the left of the input.</Prop>
      <Prop name="postfix">Content to be rendered to the right of the input.</Prop>
      <Prop name="containerProps">
...
export const WrappingTextInput = ({ onChange, ...props }) => {
...
export const StoryWrappingTextInput = () => {
  const [name, setName] = React.useState('');
  return (
    <>
      <p>
        The WrappingTextInput component renders text-based form fields that represent a single value, where the input expands to display multiple
        lines. It has accessible labels and automatic resizing. For allowable props, see <a href="#StoryTextArea">the Text Area documentation</a>. Need it to be Optimistic? Pass this as the Component for <a href="#Story_OptimisticInput">Optimistic Input</a>.
      </p>
      <CodeExample>{`<WrappingTextInput label="collection name" value={name} onChange={setName} />`}</CodeExample>
      <div style={{maxWidth: '150px'}}>
        <WrappingTextInput label="collection name" value={name} onChange={setName} />
      </div>
    </>
  );
};
...
const useInputState = (initialValue) => {
  ...
  focusconst onChange = (val) => setValue(val);
