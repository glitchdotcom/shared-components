  ...
  <ThemeContainer theme={theme}>
    {['background', 'secondaryBackground'].map(bg => (
      <Box key={bg} style={{ backgroundColor: theme.colors[bg] }} padding={1}>
        ...
        {['primary', 'secondary', 'border', 'disabled'].map(fg => (
        ...
        <Grid items={['private', 'cta', 'error', 'warning', 'info', 'success', 'selected'].map(id => ({ id }))} minWidth="4rem">
          {({ id }) => (
            <Box
              ...
              rounded>
              <P size="small">{id}</P>
              <P size="tiny" style={{ color: theme.colors[id].secondary }}>{theme.colors[id].background}</P>
            </Box>
          )}
        </Grid>
      </Box>
...
export const StoryThemePreview = () => (
  <Grid items={themeItems} minWidth="400px">
    {({ id, theme }) => (
      <Box>
        <H as="h2" mb={1}>{id}</H>
