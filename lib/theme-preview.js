...
export const ThemePreview = ({ theme }) => (
  <ThemeContainer padding={1} theme={theme}>
    {['tertiaryBackground', 'secondaryBackground', 'background'].map(bg => (
      <Box key={bg} style={{ backgroundColor: theme.colors[bg] }} padding={1}>
        ...
        <Grid items={['private', 'cta', 'error', 'warning', 'info', 'success', 'selected'].map(id => ({ id }))} minWidth="5rem">
          {({ id }) => (
            <Box style={{ color: theme.colors[id].text, backgroundColor: theme.colors[id].background, textAlign: 'center' }} padding={1} rounded>
              <P>{id}</P>
              <P>{theme.colors[id].background}</P>
            </Box>
          )}
        </Grid>
      </Box>
