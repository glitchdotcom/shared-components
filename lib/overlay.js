...
export const useOverlay = () => {
  ...
  const onClose = (e) => {
    setOpen(false);
    // not entirely sure why, but this timeout is needed to return focus to the toggle button
    // when the overlay is closed with the Escape key
...
export const Story_useOverlay_hook = () => (
  <>
    <p>The useOverlay hook provides helpers for accessibly managing the relationship between an Overlay and the button that opens it.</p>
    <CodeExample>
      {code`
        const { open, onOpen, onClose, toggleRef } = useOverlay();
        // ...
        <Button onClick={onOpen} ref={toggleRef}>Show Overlay</Button>
        // ...
        <Overlay open={open} onClose={onClose}>
          {(overlayProps) => <OverlayContent {...overlayProps} />}
        </Overlay>
      `}
    </CodeExample>
  </>
