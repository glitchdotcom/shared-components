...
export const OverlayContainer = ({ onClose, children, ...props }) => {
  React.useEffect(() => {
    const handler = (e) => {
...
export const OverlayBody = ({ children, title, onClose }) => {
  const { first, last } = useFocusTrap()
  const mountRef = useFocusOnMount()
  return (
    <div>
      <OverlayTitle>
        {title}
        <CloseButton ref={first} onClick={onClose} />
      </OverlayTitle>
      {children}
    </div>
  )
